...
MUL_23:	PULA 
		...
		LDX     #6H
CORR:  	LSL	  	M+3             ;SE DESPLAZA PRIMER BYTE
		CLRA	                ;
		ADC		#0              ;SE OBTIENE EL PRIMER CARRY
		PSHA                    ;SE GUARDA EL PRIMER CARRY EN LA PILA
		
		LSL	  	M+2             ;SE DESPLAZA SEGUNDO BYTE
		ADC		#0              ;SE OBTIENE EL SEGUNDO CARRY
		PULH
		PSHA           			;SE  GUARDA EL SEGUNDO CARRY EN AUX+0
		PSHH					;
		PULA					;		
		ADD		M+2             ;SE SUMA AL SEGUNDO BYTE EL PRIMER CARRY           
		STA		M+2             ;SE GUARDA					
		...
		ADD		M+0				;
		STA		M+0				;
		DBNZX   CORR; Pregunta si x es igual a cero si es diferente salta y decrementa

				
		
		
		MOV   M+0,INTACC1+0    ; ASIGNA PARTE ALTA DEL NUMERADOR ENTERO
		MOV   M+1,INTACC1+1    ; ""
		MOV   M+2,INTACC1+2    ; "" 
		MOV   M+3,INTACC1+3    ; ASIGNA PARTE BAJA DEL NUEMRADOR ENTERO 
...
RAIZ:   MOV   M+0,INTACC1+0    ; ASIGNA PARTE ALTA DEL NUMERADOR ENTERO
		MOV   M+1,INTACC1+1    ; ""
		MOV   M+2,INTACC1+2    ; "" 
		MOV   M+3,INTACC1+3    ; ASIGNA PARTE BAJA DEL NUEMRADOR ENTERO 
		JSR   UDVD32           ; DIVIDE SOBRE X
		MOV   INTACC1+0,AUX+0  ; CARGA PARTE ALTA COCIENTE DE DIVISION ENTERO 
		MOV   INTACC1+1,AUX+1  ; ""
		MOV   INTACC1+2,AUX+2  ; ""
		MOV   INTACC1+3,AUX+3  ; CARGA PARTE BAJA COCIENTE DE DIVISION ENTERO
		MOV   INTACC2+1,AUX+4  ; CARGA PARTE BAJA DE RESIDUO
