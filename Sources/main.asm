...
RAIZ:	
		MOV		#0H,AUX1+1		;
		MOV		#05H,AUX1+0		; USA 5 YA QUE ES MAS PROBABLE QUE LA RAIZ DE CERCA
		MOV		#05H,CONTADOR	;
		LDA		#64H			;CARGA A CON 100 AJUSTE NUMERADOR
		LDX		AUX2+0			;CARGA AUX2 CON EL MULTIPLICADOR
		MUL						;MULTIPLICA POR 100
		ADD		AUX2+1			;SUMA PARTE DECIMAL
		STA		AUX2+1			;GUARDO PARTE BAJA
		TXA						;TRANSFIERE REGISTRO X PARTE ALTA A ACUMULADOR PARTE ALTA 
		ADC		#0H				;SUMA ACARREOS
		STA		AUX2+0			;CARGA PARTE MAS ALTA

RAIX:	
		MOV     #0H,INTACC1+0	;CARGA CEROS POR SER DE 32 BITS 
		MOV     #0H,INTACC1+1	;CARGA ""
		MOV     AUX2+0,INTACC1+2;CARGA NUMERADOR
		MOV     AUX2+1,INTACC1+3;""

		MOV		AUX1+0,AUX+0		;
		MOV		AUX1+1,AUX+1		;
		
		LDA		#64H			;CARGA A CON 100------------ AJUSTE DENOMINADOR
		LDX		AUX1+0			;CARGA AUX2 CON EL MULTIPLICADOR
		MUL						;MULTIPLICA POR 100
		ADD		AUX1+1			;SUMA PARTE DECIMAL
		STA		AUX1+1			;GUARDA
		TXA						;TRANSFIERE REGISTRO X PARTE ALTA A ACUMULADOR PARTE ALTA
		ADC		#0H				;SUMA ACARREOS
		STA		AUX1+0			;GUARDA PARTE MAS ALTA
		MOV     AUX1+0,INTACC2+0;CARGA DENOMINADOR SIEMPRE SE MODIFICA EL DENOMINADOR
		MOV     AUX1+1,INTACC2+1;""
		
		JSR     UDVD32			;HACE DIVISION 
		
		LDA		INTACC1+3		;CARGA PARTE ENTERA BAJA
		PSHA					;GUARDO EN LA PILA EL DATO LUEGO SE USARA PARA DIVIDIR EL RESIDUO
		
		MOV     #0H,INTACC1+0	;CARGA CEROS POR SER DE 32 BITS 
		MOV     #64H,INTACC1+1	;CARGA ""
		
		JSR		UMULT16;
		
		MOV     #0H,INTACC1+0	;CARGA CEROS POR SER DE 32 BITS 
		MOV     #0H,INTACC1+1	;CARGA "" EL DATO YA ESTA CARGA, PUESTO QUE ES LA SALIDA DE UMILT
				
		MOV     AUX1+0,INTACC2+0;CARGA DENOMINADOR SIEMPRE SE MODIFICA EL DENOMINADOR
		MOV     AUX1+1,INTACC2+1;""
				
		JSR     UDVD32			;HACE DIVISION 2
		
		MOV		AUX+0,AUX1+0	;SE OBTINEN LOS DATOS INICIALES
		MOV		AUX+1,AUX1+1	; 
		 
		PULA					;SACO DATO ENTERO DE LA PILA
		ADD		AUX1+0			;SUMA PARTE ENTERA BAJA PARA PROMEDIO
		STA		AUX1+0			;GUARDA PARTE ENTERA ALTA		
		
		LDA		INTACC1+3		;RESIDUO EN LA BASE QUE ES PARTE BAJA UNICAMENTE
		ADD		AUX1+1			;YA CONVERTIDO
		PSHA					;PONE EL DATO GUARDADO			
		SUB   	#64H			;LE RESTA 100
		BMI   	DIVI         	;SUMA PARTES DECIMALES
		PULX					;
		CLRX					;
		PSHA					;CARGA DATO DE RESTA TIENE QUE SE POSITIVO
		LDA   AUX1+0			;
		ADD   #1H				;INCREMENTA PARTE ENTERA
		STA   AUX1+0			;CARGA NUEVA PARTE ENTERA
DIVI:	PULA
		STA	  	AUX1+1			;CARGA PARTE DECIMAL NUEVA DESPUES DE LA SUMA
		LSR		AUX1+1			;Desplaza a la derecha SUMA+2 0 -> A(7)
		LSR		AUX1+0			;SUMA+2(0)->c
		BCC		LOAD			;Pregunta si el bit de acarreo esta en 0  
		LDA		#32H			;Si (c==0) carga a con 50
		ADD		AUX1+1			;Suma la parte decimal de a con 50
		STA		AUX1+1			;Cargar la parte decimal con el resultado 
LOAD:	DBNZ CONTADOR,RAIX		;CONTADOR
